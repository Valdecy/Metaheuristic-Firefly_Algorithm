...
def firefly_algorithm(swarm_size = 3, min_values = [-5,-5], max_values = [5,5], generations = 50, alpha_0 = 0.2, beta_0 = 1, gama = 1):
    ...
    while (count <= generations):
        print("Generation: ", count, " of ", generations, " f(x) = ", position.iloc[position['Ligth_0'].idxmin(),:][-1])
        for i in range (0, swarm_size):
            for j in range(0, swarm_size):
                if (i != j):
                    ...
                    if (ligth_i > ligth_j):
                       position = update_position(position, firefly_i, firefly_j, alpha_0 = alpha_0, beta_0 = beta_0, gama = gama, firefly = i, min_values = min_values, max_values = max_values)
        count = count + 1
